...
<?php
    ...
    echo ('<form name="user_config" id="user_config" action="save_config.php" onsubmit="return ValidateInput();" method="post">
           ...
           <tr align=left><td title="Enter the Gmail address from where the FalconGate alerts will be send." colspan=2>Gmail Address: <input type=text size=20 name="mailer_address" id="mailer_address" '.$required_g.' value='.$config['main']['mailer_address'].'></td></tr>
		    <tr align=left><td title="Enter the password for Gmail address from where the FalconGate alerts will be send." colspan=2>Gmail Password: <input type="password" name="mailer_pwd" id="mailer_pwd" size="20" maxlength="500"></td></tr>
           </table>
           <br>');
